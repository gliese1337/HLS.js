...
class Stream {
  ...
  write(mem: DataView, ptr: number, len: number, pstart: number): void {
    const { payload } = this;
...
function decode_pes(mem: DataView, ptr: number, len: number, s: Stream, pstart: number): number {
...
function demux_packet(pmt: PMT, mem: DataView, ptr: number, len: number, pids: Map<number, Stream>): number {
...
function pids2streams(pids: Map<number, Stream>): Map<number, StreamData> {
...
export class TSDemuxer {
  private pmt = new PMT();
  private pids = new Map<number, Stream>();

  process(buffer: Uint8Array, offset = 0, len = buffer.byteLength): [0, Map<number, StreamData>] | [number, null] {
    const l=188;
    const { pmt, pids } = this;
    const mem = new DataView(buffer.buffer, offset + buffer.byteOffset);
  
    for (let ptr=0;;ptr+=l) {
      const length = len - ptr;
      if (!length) return [0, pids2streams(this.pids)];
      if (length<l) return [1, null]; // incompleted TS packet
  
      const n = demux_packet(pmt, mem, ptr, l, pids);
      if (n) return [n, null];  // invalid packet
    }
  }
}